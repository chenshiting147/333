...
# bgr_shape = (128, 128, 3)
# batch_size = 132#132 #max -tf.1.11.0-cuda 9
...
def main():
    ...
    image = cv2.imread(r'D:\DFLBuild\test\00000.png').astype(np.float32) / 255.0
    image2 = cv2.imread(r'D:\DFLBuild\test\00000.jpg').astype(np.float32) / 255.0
    ...
    image_bgr = cv2.cvtColor(image_lab.astype(np.uint8), cv2.COLOR_LAB2BGR)
    ...
    def t2kw_conv2d (w,b):
        return [ np.moveaxis(w.data.cpu().numpy(), [0,1,2,3], [3,2,0,1]), b.data.cpu().numpy() ]
    
    data = {}
    for i in [0,3,6,8,11,13,16,18]:
        s = 'features.%d' % i
        data[s] = t2kw_conv2d (d[ s + '.weight'], d[ s + '.bias'])
    
    import pickle
    pickled = pickle.dumps (data)
    
    with open( r'D:\DFLBuild\test\vgg11_enc_weights.npy', 'wb' ) as f:
        f.write(pickled)
    ...
    image = cv2.imread(r'D:\DFLBuild\test\sr1.png').astype(np.float32) / 255.0

    image_up = dc.upscale(image)
    cv2.imwrite (r'D:\DFLBuild\test\sr1_result.png', (image_up*255).astype(np.uint8) )
    ...
    for i in range(100):
        ...
        encoder.save_weights(r"D:\DFLBuild\test\testweights.h5")
        decoder1.save_weights(r"D:\DFLBuild\test\testweights1.h5")
        decoder2.save_weights(r"D:\DFLBuild\test\testweights2.h5")
